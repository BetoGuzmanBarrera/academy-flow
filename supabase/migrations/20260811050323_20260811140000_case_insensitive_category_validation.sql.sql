/*
# Case-insensitive category matching in create_secure_order

## Problem
The `create_secure_order` RPC validates `cart_items.details` per service,
but the category dispatch uses exact string comparison:
  `rec.category_name = 'ALEKS Universidad'`
The database currently has a category named "Aleks Universidad" (mixed case)
with 7 services. Those services fall through to the default branch, which:
  - Rejects `subject` and `partial` as unknown keys (blocks valid orders).
  - Skips required-field checks (subject, partial, quantity ranges).

## Fix
Replace the three exact-match category comparisons with case-insensitive
+ whitespace-tolerant comparisons using `upper(btrim(rec.category_name))`:
  - 'ALEKS UNIVERSIDAD'
  - 'CAMBRIDGE ONE'
  - 'FRANCÉS — BIBLIO EXOS'

## What changes
- ONLY the three `IF` / `ELSIF` category dispatch conditions.
- Everything else in the function body is byte-for-byte identical:
  - All per-service validation logic (subject, partial, quantity, etc.)
  - All JSON type checks
  - All unknown-key rejection
  - All credential validation
  - All pricing/discount calculation
  - All insert logic
...
- No permission changes (REVOKE/GRANT repeated identically).
- No price or calculation changes.
- No changes to any other function.
- No changes to frontend, Stripe, services, or credentials.
*/
...
BEGIN
  ...
  LOOP
    ...
    IF upper(btrim(rec.category_name)) = 'ALEKS UNIVERSIDAD' THEN
    ...
    ELSIF upper(btrim(rec.category_name)) = 'CAMBRIDGE ONE' THEN
      det_level := det ->> 'level';
    ...
    ELSIF upper(btrim(rec.category_name)) = 'FRANCÉS — BIBLIO EXOS' THEN
