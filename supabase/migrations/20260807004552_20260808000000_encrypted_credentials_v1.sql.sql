-- ============================================================
-- Migración: Credenciales cifradas + auditoría + rate limiter
-- Idempotente — segura de aplicar múltiples veces
-- ============================================================

-- ============================================================
-- 1. Columnas de cifrado en order_credentials
-- ============================================================
...
-- ============================================================
-- 2. Constraint de consistencia: payload e IV ambos NULL o ambos NOT NULL
-- ============================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'order_credentials_encryption_pair_check'
      AND conrelid = 'public.order_credentials'::regclass
  ) THEN
    ALTER TABLE public.order_credentials
      ADD CONSTRAINT order_credentials_encryption_pair_check
      CHECK (
        (encrypted_payload IS NULL AND encryption_iv IS NULL)
        OR
        (encrypted_payload IS NOT NULL AND encryption_iv IS NOT NULL)
      );
  END IF;
END
$$;
...
-- ============================================================
-- 3. Tabla de auditoría de acceso a credenciales
-- ============================================================
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'credential_access_log_action_check'
      AND conrelid = 'public.credential_access_log'::regclass
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'credential_access_log_credential_id_fkey'
  ) THEN
    ALTER TABLE public.credential_access_log
      ADD CONSTRAINT credential_access_log_credential_id_fkey
      FOREIGN KEY (credential_id) REFERENCES public.order_credentials(id)
      ON DELETE SET NULL;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'credential_access_log_order_id_fkey'
  ) THEN
    ALTER TABLE public.credential_access_log
      ADD CONSTRAINT credential_access_log_order_id_fkey
      FOREIGN KEY (order_id) REFERENCES public.orders(id)
      ON DELETE SET NULL;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'credential_access_log_accessed_by_fkey'
  ) THEN
    ALTER TABLE public.credential_access_log
      ADD CONSTRAINT credential_access_log_accessed_by_fkey
...
CREATE INDEX IF NOT EXISTS idx_credential_access_log_credential_id
  ON public.credential_access_log (credential_id);
CREATE INDEX IF NOT EXISTS idx_credential_access_log_accessed_by
  ON public.credential_access_log (accessed_by);
CREATE INDEX IF NOT EXISTS idx_credential_access_log_created_at
  ON public.credential_access_log (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_credential_access_log_action
  ON public.credential_access_log (action);

-- ============================================================
-- 4. RLS en credential_access_log
-- ============================================================

ALTER TABLE public.credential_access_log ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "admins_read_credential_access_log"
  ON public.credential_access_log;

CREATE POLICY "admins_read_credential_access_log"
  ON public.credential_access_log FOR SELECT
  TO authenticated
  USING (public.is_admin());

-- ============================================================
-- 5. Grants para credential_access_log
-- ============================================================

REVOKE ALL ON public.credential_access_log FROM anon, authenticated, PUBLIC;
GRANT SELECT ON public.credential_access_log TO authenticated;
GRANT INSERT ON public.credential_access_log TO service_role;

-- ============================================================
-- 6. Revocar acceso directo a order_credentials
-- ============================================================

REVOKE ALL ON public.order_credentials FROM anon, authenticated, PUBLIC;

DROP POLICY IF EXISTS "Admins can manage order credentials"
  ON public.order_credentials;
DROP POLICY IF EXISTS "Users can update credentials for own orders"
  ON public.order_credentials;
DROP POLICY IF EXISTS "Users can view credentials for own orders"
  ON public.order_credentials;

ALTER TABLE public.order_credentials FORCE ROW LEVEL SECURITY;

GRANT SELECT, INSERT, UPDATE ON public.order_credentials TO service_role;

CREATE INDEX IF NOT EXISTS idx_order_credentials_order_active
  ON public.order_credentials (order_id)
  WHERE deleted_at IS NULL;

-- ============================================================
-- 7. Tabla de rate limiting durable
-- ============================================================

CREATE TABLE IF NOT EXISTS public.credential_reveal_requests (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_credential_reveal_requests_admin_created
  ON public.credential_reveal_requests (admin_id, created_at DESC);

ALTER TABLE public.credential_reveal_requests ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.credential_reveal_requests FORCE ROW LEVEL SECURITY;
REVOKE ALL ON public.credential_reveal_requests FROM anon, authenticated, PUBLIC;
GRANT SELECT, INSERT, DELETE ON public.credential_reveal_requests TO service_role;

-- ============================================================
-- 8. Función atómica de rate limiting con pg_advisory_xact_lock
-- ============================================================
...
BEGIN
  ...
  v_lock_key := hashtextextended(p_admin_id::text, 0);

  PERFORM pg_advisory_xact_lock(v_lock_key);

  SELECT count(*) INTO v_count
  FROM public.credential_reveal_requests
  ...
  INSERT INTO public.credential_reveal_requests (admin_id, request_id)
  VALUES (p_admin_id, p_request_id);

  DELETE FROM public.credential_reveal_requests
...
REVOKE EXECUTE ON FUNCTION public.check_reveal_rate_limit(uuid, text)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.check_reveal_rate_limit(uuid, text)
  TO service_role;

-- ============================================================
-- 9. complete_order_secure
-- ============================================================
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'La orden no existe o no está pendiente';
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.complete_order_secure(uuid, uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.complete_order_secure(uuid, uuid)
  TO service_role;

-- ============================================================
-- 10. reopen_order_secure
-- ============================================================
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'La orden no existe o no está completada';
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.reopen_order_secure(uuid, uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.reopen_order_secure(uuid, uuid)
  TO service_role;

-- ============================================================
-- 11. cleanup_expired_credentials
-- ============================================================
...
BEGIN
  ...
  SELECT id, order_id, NULL,
    'expired', true, 'cron_cleanup', NULL
  FROM expired;
...
REVOKE EXECUTE ON FUNCTION public.cleanup_expired_credentials()
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.cleanup_expired_credentials()
  TO service_role;

-- ============================================================
-- 12. create_secure_order RPC
-- ============================================================
...
BEGIN
  ...
  FOR cred_entry IN SELECT value FROM jsonb_array_elements(p_encrypted_credentials) AS entry(value)
  LOOP
    IF jsonb_typeof(cred_entry) IS DISTINCT FROM 'object' THEN
      RAISE EXCEPTION 'Cada entrada de credenciales debe ser un objeto';
    END IF;

    IF jsonb_typeof(cred_entry -> 'credential_id') IS DISTINCT FROM 'string' THEN
      RAISE EXCEPTION 'credential_id debe ser texto';
    END IF;
    IF (cred_entry ->> 'credential_id') !~ v_uuid_regex THEN
      RAISE EXCEPTION 'El identificador de credencial no es un UUID válido';
    END IF;

    IF jsonb_typeof(cred_entry -> 'service_id') IS DISTINCT FROM 'string' THEN
      RAISE EXCEPTION 'service_id debe ser texto';
    END IF;
    IF (cred_entry ->> 'service_id') !~ v_uuid_regex THEN
      RAISE EXCEPTION 'El identificador de servicio en las credenciales no es válido';
    END IF;

    IF jsonb_typeof(cred_entry -> 'encrypted_payload') IS DISTINCT FROM 'string' THEN
      RAISE EXCEPTION 'encrypted_payload debe ser texto';
    END IF;
    IF cred_entry ->> 'encrypted_payload' = '' THEN
      RAISE EXCEPTION 'El payload cifrado no puede estar vacío';
    END IF;
    IF length(cred_entry ->> 'encrypted_payload') > 8192 THEN
      RAISE EXCEPTION 'El payload cifrado excede el tamaño máximo permitido';
    END IF;

    BEGIN
      PERFORM decode(cred_entry ->> 'encrypted_payload', 'base64');
    EXCEPTION WHEN others THEN
      RAISE EXCEPTION 'El payload cifrado no es base64 válido';
    END;

    IF jsonb_typeof(cred_entry -> 'encryption_iv') IS DISTINCT FROM 'string' THEN
      RAISE EXCEPTION 'encryption_iv debe ser texto';
    END IF;
    IF cred_entry ->> 'encryption_iv' = '' THEN
      RAISE EXCEPTION 'El IV no puede estar vacío';
    END IF;

    BEGIN
      decoded_iv := decode(cred_entry ->> 'encryption_iv', 'base64');
    EXCEPTION WHEN others THEN
    ...
    IF jsonb_typeof(cred_entry -> 'key_version') IS DISTINCT FROM 'number' THEN
      RAISE EXCEPTION 'key_version debe ser un número';
    END IF;
    IF (cred_entry ->> 'key_version')::int <> 1 THEN
    ...
    FROM jsonb_object_keys(cred_entry) AS key;
  ...
  IF EXISTS (
    SELECT 1
    FROM (
      SELECT cred_entry ->> 'credential_id' AS cid, count(*) AS cnt
      FROM jsonb_array_elements(p_encrypted_credentials) AS entry(cred_entry)
      GROUP BY cred_entry ->> 'credential_id'
      HAVING count(*) > 1
    ) dup
  ) THEN
  ...
  IF EXISTS (
    SELECT 1
    FROM (
      SELECT cred_entry ->> 'service_id' AS sid, count(*) AS cnt
      FROM jsonb_array_elements(p_encrypted_credentials) AS entry(cred_entry)
      GROUP BY cred_entry ->> 'service_id'
      HAVING count(*) > 1
    ) dup
  ) THEN
...
-- ============================================================
-- 13. Revocar create_pending_order
-- ============================================================

REVOKE EXECUTE ON FUNCTION public.create_pending_order(text, text, jsonb, jsonb)
  FROM PUBLIC, anon, authenticated;
